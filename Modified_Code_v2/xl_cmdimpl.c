    ...
    int domid)
{
    ...
    if (domid < 0) {
	//RAVEN_EDP_M
        printf("%-33s %4s %4s %9s %9s %11s\n", "Name", "ID", "VCPU", "Period", "Budget", "Deadline");
        return 0;
    }
    ...
    {
	//RAVEN_EDP_M
        printf("%-33s %4d %4d %9"PRIu64" %9"PRIu64" %9"PRIu64"\n",
            domname,
            domid,
...
int main_sched_rt(int argc, char **argv)
{
    ...
    //RAVEN_EDP_M
    SWITCH_FOREACH_OPT(opt, "d:p:b:z:v:c:h", opts, "sched-rt", 0) {
    case 'd':
        dom = optarg;
        break;
    case 'p':
    ...
    if (cpupool && (dom || opt_p || opt_b || opt_z || opt_v)) {
        fprintf(stderr, "Specifying a cpupool is not allowed with other options.\n");
        return 1;
    }
    if (!dom && (opt_p || opt_b || opt_z || opt_v)) {
        fprintf(stderr, "Must specify a domain.\n");
        return 1;
    }
    //RAVEN_EDP_M
    if ( (opt_v || opt_p || opt_b || opt_z) && (opt_p + opt_b + opt_v + opt_z != 4) ) {
        fprintf(stderr, "Must specify vcpu, period, budget, deadline\n");
        return 1;
    }
    //RAVEN_EDP_M
    ...
    if (!dom) { /* list all domain's rt scheduler info */
        ...
        uint32_t domid = find_domain(dom);
	//RAVEN_EDP_M
        if (!opt_p && !opt_b && !opt_v && !opt_z) { /* output rt scheduler info */
