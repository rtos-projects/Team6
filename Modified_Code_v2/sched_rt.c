...
//RAVEN_EDP
#define RT_DEFAULT_DEADLINE     (MICROSECS(4))
...
struct rt_vcpu {
    ...
    //RAVEN_EDP
    s_time_t cur_period;      /* current period for EDF */
...
rt_dump_vcpu(struct rt_vcpu *svc)
{
    ...
    //RAVEN_EDP_M
    printk("[%5d.%-2d] cpu %d, (%"PRId64", %"PRId64", %"PRId64"), cur_p=%"PRId64" cur_b=%"PRId64" cur_d=%"PRId64" last_start=%"PRId64" onR=%d runnable=%d cpu_hard_affinity=%s ",
...
rt_alloc_vdata(const struct scheduler *ops, struct vcpu *vc, void *dd)
{
    ...
    svc->cur_period += count * MICROSECS(svc->deadline);
    svc->cur_deadline = svc->cur_period - (MICROSECS(svc->period) - MICROSECS(svc->deadline));
    //RAVEN_EDP
...
burn_budgets(const struct scheduler *ops, struct rt_vcpu *svc, s_time_t now) 
{
     ...
     */
    delta = now - svc->cur_period;
    if ( delta >= 0 ) 
    {
        //RAVEN_EDP_M
        count = ( delta/MICROSECS(svc->period) ) + 1;
        svc->cur_period += count * MICROSECS(svc->period);
        //RAVEN_EDP_M
        svc->cur_deadline = svc->cur_period - (MICROSECS(svc->period) - MICROSECS(svc->deadline));
...
rt_schedule(const struct scheduler *ops, s_time_t now, bool_t tasklet_work_scheduled)
{
    ...
    if(counter > 0)
    {
      printk("Now Processing domain [domain-id vcpu-id] [%5d.%-2d]\n", scurr->vcpu->domain->domain_id, scurr->vcpu->vcpu_id);
      counter--;
    }
