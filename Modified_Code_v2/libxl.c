                               ...
                               libxl_domain_sched_params *scinfo)
{
	...
	//RAVEN_EDP
	scinfo->rt.vcpus[i].deadline = sdom[i].deadline;
                                               ...
                                               const uint16_t num_vcpus)
{
    ...
    if (vcpu_index == LIBXL_DOMAIN_SCHED_PARAM_VCPU_DEFAULT ||
        ...
        scinfo->rt.budget == LIBXL_DOMAIN_SCHED_PARAM_DEADLINE_DEFAULT)
    //RAVEN_EDP_M    
    {
        return 1;
    }
    ...
    //RAVEN_EDP
    if (scinfo->rt.deadline < 1 ||
        scinfo->rt.deadline > SCHED_RT_VCPU_PERIOD_MAX)
    {
        LOG(ERROR, "VCPU deadline is not set or out of range, "
                    "valid values are within range from 0 to %lu", SCHED_RT_VCPU_PERIOD_MAX);
        return 2;
    }


    if (scinfo->rt.deadline < scinfo->rt.budget)
    {
        LOG(ERROR, "VCPU budget cannot be greater than deadline.");
        return 2;
    }
    //RAVEN_EDP
                               ...
                               const libxl_domain_sched_params *scinfo)
{
	...
	//RAVEN_EDP
	sdom.deadline = scinfo->rt.deadline;
